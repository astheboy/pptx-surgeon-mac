class PPTXSurgeonUI {
    constructor() {
        this.currentFile = null; // Object: { path: string, name: string }
    ...
    setupIpcListeners() {
        window.electron.on('file-selected', (result) => {
            if (result && result.path) {
                this.currentFile = { path: result.path, name: result.name };
                this.originalFileName = result.name;
                this.showFileInfo(result.name);
                this.activateStep(2);
                this.showToast('파일 준비 완료!', 'success');
            }
        });
        window.electron.on('update:progress', ({ progress, message }) => this.updateProgress(progress, message));
        window.electron.on('update:verbose', ({ message }) => this.showVerboseLog(message));
        window.electron.on('update:error', ({ message }) => {
    ...
    async selectFileByDialog() {
        try {
            ...
            this.showToast('파일 선택 중 오류 발생: ' + error.message, 'error');
        }
    }
    ...
    async handleFileDrop(file) {
        ...
        try {
            const result = await window.electron.invoke('handle:file-drop', file.path);
            if (result && result.path) {
                this.currentFile = { path: result.path, name: result.name };
                this.originalFileName = result.name;
                this.showFileInfo(result.name);
             ...
             this.showToast('파일 처리 오류: ' + error.message, 'error');
        }
    }
    ...
    async analyzeFile() {
        ...
        try {
            ...
            const result = await window.electron.invoke('run:analyze', this.currentFile.path);
            if (result.success) {
    ...
    async processFile() {
        ...
        try {
            ...
            this.showProgress();
            const result = await window.electron.invoke('run:process', { 
                filePath: this.currentFile.path,
                originalName: this.originalFileName,
                options: options
            });
            if (result.success) {
                this.showResult(result.outputPath);
    ...
    async showResult(processedFilePath) {
        this.hideProgress();
        document.getElementById('resultContainer').style.display = 'block';
        const downloadBtn = document.getElementById('downloadBtn');
        downloadBtn.onclick = async () => {
            try {
                const savedPath = await window.electron.invoke('dialog:saveFile', processedFilePath);
                if (savedPath) {
                    this.showToast('파일 저장 완료: ' + savedPath, 'success');
                } else {
                    this.showToast('파일 저장이 취소되었습니다.', 'info');
                }
            } catch (error) {
                this.showToast('파일 저장 오류: ' + error.message, 'error');
            }
        };
        this.updateLanguage();
    }
    ...
    updateLanguage() {
        ...
        if(selectFileBtn) selectFileBtn.style.display = 'none'; // Hide old button
